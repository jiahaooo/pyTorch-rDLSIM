...
from utils.option import AI_parse, dict2str
...
def main(json_path, opt=None):
    ...
    if opt['G_scheduler_type'] == 'MultiStepLR':
        opt['epoch_num'] = math.ceil(1 + (opt['G_scheduler_milestones'][-1] + opt['P_scheduler_milestones'][-1] - current_step) / (len(train_set) // opt['dataloader_batch_size']))
    elif opt['G_scheduler_type'] == 'CosineAnnealingLR':
        opt['epoch_num'] = math.ceil(1 + (opt['G_scheduler_IterNum'] + opt['P_scheduler_IterNum'] - current_step) / (len(train_set) // opt['dataloader_batch_size']))
    else:
        raise RuntimeError

    G_loss_idx, G_loss_list, G_lr_idx, G_lr_list = [], [], [], []
    P_loss_list, P_loss_idx, P_lr_list, P_lr_idx = [], [], [], []
    psnr_ssim_idx = []
    ...
    report_total_raw_psnr, report_total_raw_nrmse, report_total_raw_ssim, report_total_raw_msssim = 0.0, 100.0, 0.0, 0.0
    report_total_sim_psnr, report_total_sim_nrmse, report_total_sim_ssim, report_total_sim_msssim = 0.0, 100.0, 0.0, 0.0
    ...
    for epoch in range(opt['epoch_num']):
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['num_iter_interval_print'] == 0:
                ...
                current_lr = model.current_learning_rate()  # G, P
                message = '{}  <epoch:{:3d}, iter:{:8,d}, G_lr:{:.3e}, P_lr:{:.3e}>'.format(get_timestamp(), epoch, current_step, current_lr['G'], current_lr['P'])
                for k, v in logs.items():  # merge log information into message
                    message += '{:s}: {:.3e} '.format(k, v)
                outinfo.info(message + '\n')
                if current_step <= opt['P_pretrain_iternum']:
                    P_loss_list.append(logs['P_loss'])
                    P_loss_idx.append(current_step)
                    P_lr_list.append(current_lr['P'])
                    P_lr_idx.append(current_step)
                else:
            ...
            # 5) save model
            # -------------------------------
            if current_step < opt['P_pretrain_iternum'] and (current_step % opt['num_iter_interval_save'] == 0):
                outinfo.info('Saving P model, at iter {}\n'.format(current_step))
                model.save(current_step, 'P')
            elif current_step == opt['P_pretrain_iternum']:
                outinfo.info('Saving P latest model, at iter {}\n'.format(current_step))
                model.save('latest', 'P')
            elif current_step == 1 and opt['P_pretrain_iternum'] == 0:
                outinfo.info('Copy P pertrained model, at iter {}\n'.format(current_step))
                model.save('pertrained', 'P')
            elif current_step > opt['P_pretrain_iternum'] and ((current_step - opt['P_pretrain_iternum']) % opt['num_iter_interval_save'] == 0):
                outinfo.info('Saving G model, at iter {}\n'.format(current_step))
                model.save(current_step, 'G')
                model.save(current_step, 'P')

            # -------------------------------
            # 6) testing
            # -------------------------------
            if current_step > opt['P_pretrain_iternum'] and ((current_step - opt['P_pretrain_iternum']) % opt['num_iter_interval_val'] == 0):
                ...
                for test_data in val_loader:
                    ...
                    need_input = True if current_step - opt['P_pretrain_iternum'] == opt['num_iter_interval_val'] else False
                    ...
                    def check_and_write(key, save_append=''):
                        ...
                        if visuals[key] is not None:
                            sampling_rate = visuals['raw_sampling_rate'] if key.find('raw') >= 0 else visuals['sim_sampling_rate']
                            write_mrc_image(visuals[key], os.path.join(img_dir, '{:s}_{}.mrc'.format(img_name, save_label)), sampling_rate=sampling_rate)

                    if current_step - opt['P_pretrain_iternum'] == opt['num_iter_interval_val']:
                    ...
                    # -----------------------
                    if if_val_need_gt:
                        current_raw_nrmse, current_raw_psnr, current_raw_ssim, current_raw_msssim = calculate_index(visuals['raw_target_data'], visuals['raw_infer_data'], border=16)
                        outinfo.info('RAW -> {:>10s} | psnr:{:<5.2f}dB | nrmse:{:<5.4f} | ssim:{:<5.4f} | msssim:{:<5.4f}\n'.format(image_name_ext, current_raw_psnr, current_raw_nrmse,
                                                                                                                                    current_raw_ssim, current_raw_msssim))

                        raw_psnr_list.append(current_raw_psnr)
                        raw_ssim_list.append(current_raw_ssim)
                        raw_msssim_list.append(current_raw_msssim)
                        raw_nrmse_list.append(current_raw_nrmse)
                        ...
                        current_sim_nrmse, current_sim_psnr, current_sim_ssim, current_sim_msssim = calculate_index(visuals['sim_target_data'], visuals['sim_infer_data'],
                                                                                                                                 border=32)
                        outinfo.info(
                            'SIM -> {:>10s} | psnr:{:<5.2f}dB | nrmse:{:<5.4f} | ssim:{:<5.4f} | msssim:{:<5.4f}\n'.format(image_name_ext, current_sim_psnr,
                                                                                                                                                 current_sim_nrmse, current_sim_ssim,
                                                                                                                                                 current_sim_msssim))
                ...
                if if_val_need_gt:
                    ...
                    all_raw_psnr_list.append(raw_psnr_list)
                    all_raw_ssim_list.append(raw_ssim_list)
                    all_raw_msssim_list.append(raw_msssim_list)
                    all_raw_nrmse_list.append(raw_nrmse_list)
                    avg_raw_psnr, avg_raw_ssim, avg_raw_msssim, avg_raw_nrmse = np.mean(raw_psnr_list), np.mean(raw_ssim_list), np.mean(raw_msssim_list), np.mean(raw_nrmse_list)
                    avg_raw_psnr_list.append(avg_raw_psnr)
                    avg_raw_ssim_list.append(avg_raw_ssim)
                    avg_raw_msssim_list.append(avg_raw_msssim)
                    avg_raw_nrmse_list.append(avg_raw_nrmse)
                    outinfo.info(
                        'RAW --> avg_psnr:{:<5.2f}dB, avg_nrmse:{:<5.4f}, avg_ssim:{:<5.4f}, avg_msssim:{:<5.4f}\n'.format(avg_raw_psnr, avg_raw_nrmse, avg_raw_ssim, avg_raw_msssim))
                    ...
                    outinfo.info(
                        'SIM --> avg_psnr:{:<5.2f}dB, avg_nrmse:{:<5.4f}, avg_ssim:{:<5.4f}, avg_msssim:{:<5.4f}\n'.format(avg_sim_psnr, avg_sim_nrmse, avg_sim_ssim, avg_sim_msssim))

                    psnr_ssim_idx.append(current_step)

                    if report_total_raw_psnr + 0.05 < avg_raw_psnr:
                        report_total_raw_psnr = avg_raw_psnr
                        model.save('PSNR_RawDenoiseBEST', 'G')
                        outinfo.info('save rawdenoising-psnr-best model | avg_psnr:{:<5.2f}dB | {:0>6}\n'.format(avg_raw_psnr, current_step))
                    if report_total_raw_ssim + 0.005 < avg_raw_ssim:
                        report_total_raw_ssim = avg_raw_ssim
                        model.save('SSIM_RawDenoiseBEST', 'G')
                        outinfo.info('save rawdenoising-ssim-best model | avg_ssim:{:<5.4f} | {:0>6}\n'.format(avg_raw_ssim, current_step))

                    if report_total_sim_psnr + 0.05 < avg_sim_psnr:
                        report_total_sim_psnr = avg_sim_psnr
                        model.save('PSNR_SIReconBEST', 'G')
                        outinfo.info('save sirecon-psnr-best model | avg_psnr:{:<5.2f}dB | {:0>6}\n'.format(avg_sim_psnr, current_step))
                    if report_total_sim_ssim + 0.005 < avg_sim_ssim:
                        report_total_sim_ssim = avg_sim_ssim
                        model.save('SSIM_SIReconBEST', 'G')
                        outinfo.info('save sirecon-ssim-best model | avg_ssim:{:<5.4f} | {:0>6}\n'.format(avg_sim_ssim, current_step))

            if (current_step % opt['num_iter_interval_val'] == 0) or ((current_step - opt['P_pretrain_iternum']) % opt['num_iter_interval_val'] == 0):
                # plot
                fig = plt.figure()
                # G - lr
                if len(G_lr_list) > 0:
                ...
                if len(P_lr_list) > 0:
                    plt.subplot(242)
                    plt.plot(P_lr_idx, P_lr_list, color='black', marker='o')
                    plt.title('P_lr')
                # G - loss
                if len(G_loss_list) > 0:
                    ...
                    plt.title('G_loss')
                # P - loss
                if len(P_loss_list) > 0:
                    plt.subplot(244)
                    plt.plot(P_loss_idx, P_loss_list, color='black', marker='o')
                    plt.title('P_loss')
                # RAW - PSNR
    ...
    if if_val_need_gt and len(avg_sim_psnr_list) >= 1:
        avg_raw_psnr_list = np.array(avg_raw_psnr_list)
        avg_raw_ssim_list = np.array(avg_raw_ssim_list)
        avg_raw_msssim_list = np.array(avg_raw_msssim_list)
        avg_raw_nrmse_list = np.array(avg_raw_nrmse_list)
        ...
        outinfo.info(
            'BEST Raw  PSNR :{:<5.2f}dB at iter {:0>6}\n'.format(avg_raw_psnr_list.max(), opt['P_pretrain_iternum'] + opt['num_iter_interval_val'] * avg_raw_psnr_list.argmax()))
        outinfo.info('BEST Raw  SSIM :{:<5.4f} at iter {:0>6}\n'.format(avg_raw_ssim_list.max(), opt['P_pretrain_iternum'] + opt['num_iter_interval_val'] * avg_raw_ssim_list.argmax()))
        outinfo.info(
            'BEST Raw  NRMSE:{:<5.4f} at iter {:0>6}\n'.format(avg_raw_nrmse_list.min(), opt['P_pretrain_iternum'] + opt['num_iter_interval_val'] * avg_raw_nrmse_list.argmin()))
        outinfo.info(
            'BEST Raw MSSSIM:{:<5.4f} at iter {:0>6}\n'.format(avg_raw_msssim_list.max(), opt['P_pretrain_iternum'] + opt['num_iter_interval_val'] * avg_raw_msssim_list.argmax()))
        outinfo.info(
            'BEST SIM  PSNR :{:<5.2f}dB at iter {:0>6}\n'.format(avg_sim_psnr_list.max(), opt['P_pretrain_iternum'] + opt['num_iter_interval_val'] * avg_sim_psnr_list.argmax()))
        outinfo.info('BEST SIM  SSIM :{:<5.4f} at iter {:0>6}\n'.format(avg_sim_ssim_list.max(), opt['P_pretrain_iternum'] + opt['num_iter_interval_val'] * avg_sim_ssim_list.argmax()))
        outinfo.info(
            'BEST SIM  NRMSE:{:<5.4f} at iter {:0>6}\n'.format(avg_sim_nrmse_list.min(), opt['P_pretrain_iternum'] + opt['num_iter_interval_val'] * avg_sim_nrmse_list.argmin()))
        outinfo.info(
            'BEST SIM MSSSIM:{:<5.4f} at iter {:0>6}\n'.format(avg_sim_msssim_list.max(), opt['P_pretrain_iternum'] + opt['num_iter_interval_val'] * avg_sim_msssim_list.argmax()))
    ...
    if len(P_loss_list) >= 1:
        np.save(os.path.join(opt['save_path'], 'npyfiles', 'P_loss_idx.npy'), P_loss_idx)
        np.save(os.path.join(opt['save_path'], 'npyfiles', 'P_loss_list.npy'), P_loss_list)
...
if __name__ == '__main__':
    main('../AI_options/train_rdl.json')
